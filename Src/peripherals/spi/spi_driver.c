...
__INLINE void SPI_en_I2S(SPI_driver_t* self){
    bit_band_write((uint32_t)&self->SPI_I2SCFGR, SPI_I2SE_BIT, 1);
}

__INLINE void SPI_dis_I2S(SPI_driver_t *self){
...
__INLINE void SPI_set_I2S_conf(SPI_driver_t* self, I2S_conf_t conf){
...
__INLINE void SPI_set_I2S_std(SPI_driver_t* self, I2S_std_t std){
    uint32_t reg_val = self->SPI_I2SCFGR;
...
__INLINE void SPI_set_I2S_data_len(SPI_driver_t* self, I2S_data_len_t len){
    uint32_t reg_val = self->SPI_I2SCFGR;
...
__INLINE void SPI_set_I2S_chan_len(SPI_driver_t* self, I2S_chan_len_t len){
    uint32_t addr = (uint32_t)&self->SPI_I2SCFGR;
    switch(len){
...
__INLINE void SPI_set_I2S_ckpol(SPI_driver_t* self, I2S_ckpol_t val){
    uint32_t addr = (uint32_t)&self->SPI_I2SCFGR;
    switch(val){
        case I2S_CKPOL_LOW:
